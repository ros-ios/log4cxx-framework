...
echo "build for ios 32bit"
if (! xcodebuild -configuration Release -target ALL_BUILD -sdk iphoneos ARCHS="${ARCHS_IOS_32BIT}" ONLY_ACTIVE_ARCH=NO clean build TARGET_BUILD_DIR="${IOS32}" $FORMATTER )
    then
        exit 1
fi

echo "build for ios 64bit"
if (! xcodebuild -configuration Release -target ALL_BUILD -sdk iphoneos ARCHS="${ARCHS_IOS_64BIT}" ONLY_ACTIVE_ARCH=NO clean build TARGET_BUILD_DIR="${IOS64}" $FORMATTER )
    then
        exit 1
fi
...
echo "build for simulator 32bit"
if (! xcodebuild -configuration Release -target ALL_BUILD -sdk iphonesimulator ARCHS="${ARCHS_SIM_32BIT}" ONLY_ACTIVE_ARCH=NO clean build TARGET_BUILD_DIR="${SIM32}" $FORMATTER )
    then
        exit 1
fi

echo "build for simulator 64bit"
if (! xcodebuild -configuration Release -target ALL_BUILD -sdk iphonesimulator ARCHS="${ARCHS_SIM_64BIT}" ONLY_ACTIVE_ARCH=NO clean build TARGET_BUILD_DIR="${SIM64}" $FORMATTER )
    then
        exit 1
fi
